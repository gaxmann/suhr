...
The `"hoursh"` entry is the short form for hours, like `h`. `"daysh"` is the short form for days, like `d`. `"oclocksh"` is the short form for times without seconds, for example `12:34h`; you can leave it empty if such a notation is not used in your language. 
